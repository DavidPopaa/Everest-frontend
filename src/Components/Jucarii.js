...
const Jucarii = () => {
  ...
  useEffect(() => {
    const getAllData = async () => {
      try {
        const response = await fetch("https://everest-app-api.onrender.com/api/sell/jucarii");
  ...
  return (
    <div className="all-container">
      <div className="all-big-title">Jucarii</div>
            ...
            return (
              <div key={item._id} className="all-parent-div">
                <div>
                  ...
                  <div className="all-pric all-small">Pret total (target): {item.pret} RON</div>
                  {item.unitate === 0 ? (
                  ...
                  <div className="all-price">
                    Suma stransa: {Math.round(item.suma_stransa * 100) / 100} RON
                  </div>                
                ...
                <div className="all-small">Utilizatori inscrisi ({item.asigned_users.length}):</div>
